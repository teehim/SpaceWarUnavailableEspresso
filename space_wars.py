...
NEW_LABELS = {1: 'Hardcore Achiever', 3: 'Hardcore Killer', 2: 'Casual Achiever', 0: 'Casual Killer'}

classes = []
model = joblib.load('./model/new_model.h5')
...
def prediction_new_user_type(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count):
	global B0, B1, B2, B3, B4, B5, B6, B7, B8, B9, B10
	b0 = statistics.mean(B0[((len(B0) - 5)  if len(B0) >= 5 else 0):]) if len(B0) else 0
	b1 = statistics.mean(B1[((len(B1) - 5)  if len(B1) >= 5 else 0):]) if len(B1) else 0
	b2 = coin_count - B2
	b3 = destroyed_enemy_count - B3
	b4 = shots_count - B4
	b5 = b4 - b3
	b6 = level
	b7 = keyX_pressed_count - B7
	b8 = keyY_pressed_count - B8
	b9 = respawn_enemy_count - B9
	b10 = respawn_coin_count - B10
	X = {0:b0, 1:b1, 2:b2, 3:b3, 4:b4, 5:b5, 6:b6, 7:b7, 8:b8, 9:b9, 10:b10}

	y = model.predict_one(X)
	classes.append(NEW_LABELS.get(y))
	return NEW_LABELS.get(y)
...
def show_score(score, level, name, coin_count, font_size = 16, x=10, y=10, game_over=False, user_type=None, new_user_type=None):
	...
	if new_user_type:
		new_user_type_text = score_font.render("(NEW) You are : " + str(new_user_type), True, (255, 255, 0))
...
def show_game_over(screen_sizeX, screen_sizeY, score, high_score, coin_count, user_type):
	...
	message_display_center('You\'re a ' + user_type, font_medium, yellow, int(screen_sizeX/2), int(screen_sizeY * 3/10))
...
B0 = [] # A0) Position in X axis => position X [1, 2, 3, 2, 1] / 5
B1 = [] # A1) Position in Y axis => position Y [200, 150, 130, 170] / 4
B2 = 0  # A2) Number of coins collected => Total
B3 = 0  # A3) Number of destroyed enemies => Total
B4 = 0  # A4) Number of shots => Total
B5 = 0  # A5) Number of shots without enemies => Total (A4 - A3)
B6 = 0  # A6) Level reach
B7 = 0  # A7) key X pressed count
B8 = 0  # A8) key Y pressed count
B9 = 0  # A9) Number of enemy created
B10 = 0  # A10) Number of coin created


def thread_collect_data():
	global thread, A0, A1, A2, A3, A4, A5, B0, B1, player
...
def save_batch_collection_data(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count):
	global B0, B1, B2, B3, B4, B5, B6, B7, B8, B9, B10
	b0 = statistics.mean(B0[((len(B0) - 5)  if len(B0) >= 5 else 0):]) if len(B0) else 0
	b1 = statistics.mean(B1[((len(B1) - 5)  if len(B1) >= 5 else 0):]) if len(B1) else 0
	b2 = coin_count - B2
	...
	b10 = respawn_coin_count - B10
	B2 = coin_count
	B3 = destroyed_enemy_count
	B4 = shots_count
	B7 = keyX_pressed_count
	B8 = keyY_pressed_count
	B9 = respawn_enemy_count
	B10 = respawn_coin_count
	if sum([b0, b1, b2, b3, b4, b5]) != 0:
		with open("batch_train_data.txt", "a") as file_object:
			file_object.write(",".join(map(str, [b0, b1, b2, b3, b4, b5, b6, b7, b8, b9, b10])))
			file_object.write("\n")
...
#			score += level_score_increase
		...
		if game_over:
			...
			if (datetime.now() - time_count).seconds >= 5:
				time_count = datetime.now()
				new_user_type = prediction_new_user_type(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count)
				save_batch_collection_data(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count)
				print(datetime.now())
